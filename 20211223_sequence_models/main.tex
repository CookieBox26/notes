...
\newcommand*{\mywatermark}{\addfontfeatures{Color=PaleVioletRed} \textbf{\small DRAFT 2022-01-15 \\ \url{https://github.com/CookieBox26/notes/tree/main/20211223_sequence_models} }}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item データ間でセルフ(?)アテンションする \cite{JannikKossen2021}。
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item セルフアテンションを Softmax しない Transformer で偏微分方程式を解く \cite{ShuhaoCao2021}。
    \end{itemize}
  \end{itemize}
\end{itemize}
\end{PROP}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item ドロップアウトを活用して LSTM の計算量を削減する \cite{AnupSarma2021}。
  \end{itemize}
\end{itemize}
\end{PROP}
\vspace{1pt}
...
\begin{itemize}
  ...
  \item ニューラル時系列モデルにおいて誤差の自己相関を調整する \cite{FanKengSun2021}。
...
\vspace{1pt}
\begin{PROP}[colback=White, left=0pt, borderline={0.5mm}{0mm}{white,dashed}]{グループ「Transformer」のサブグループ「セルフアテンションの計算量に対処する」}
\begin{itemize}
...
\renewcommand*{\mysubsectiontitle}{セルフアテンションを Softmax しない Transformer で偏微分方程式を解く}
...
\renewcommand*{\mysubsectiontitle}{データ間でセルフ(?)アテンションするNon-Parametric Transformer}
